...
def get_token(filepath):
    ...
    if len(local_list) != 0:    
        print(f'local_list.shape:{len(local_list)}\nlocal_list:{local_list}')
    ...
    if len(pointer_list) != 0:
        ...
        pointer_list = create_token(pointer_list)
        
    ret_list = list(np.array(ret_list) + np.array(array_number_list) + np.array(array_chara_list) + np.array(pointer_list))
    print('完了！')
    return ret_list
